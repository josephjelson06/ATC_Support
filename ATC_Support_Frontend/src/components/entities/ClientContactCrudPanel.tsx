...
import type { ApiClientContact } from '../../lib/types';

type ClientContactCrudPanelProps = {
  mode: 'create' | 'edit';
  clientId: number;
  contact?: Pick<ApiClientContact, 'id' | 'name' | 'email' | 'phone' | 'designation' | 'isPrimary'>;
  onCompleted?: (contact: ApiClientContact, mode: 'create' | 'edit') => void | Promise<void>;
...
export function ClientContactCrudPanel({ mode, clientId, contact, onCompleted, onDeleted }: ClientContactCrudPanelProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const saved =
        mode === 'create'
          ? await apiFetch<ApiClientContact>(`/clients/${clientId}/contacts`, { method: 'POST', body: payload })
          : await apiFetch<ApiClientContact>(`/contacts/${contact?.id}`, { method: 'PATCH', body: payload });
  ...
  const handleDelete = async () => {
    ...
    try {
      await apiFetch<void>(`/contacts/${contact.id}`, { method: 'DELETE' });
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-bold text-slate-900">Client contact</p>
            <p className="mt-1">Contacts are used for ticket requester context and follow-ups.</p>
          </div>
        </div>
      </div>
      ...
      <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
        <input type="checkbox" checked={form.isPrimary} onChange={handleChange('isPrimary')} className="h-4 w-4 rounded border-slate-300 text-orange-600" />
        <span className="font-semibold">Mark as primary contact</span>
      </label>
