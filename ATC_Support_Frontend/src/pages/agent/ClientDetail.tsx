...
export default function ClientDetail() {
  ...
  const openCreateContactModal = () => {
    openModal({
      title: `Add Contact to ${client.name}`,
      size: 'lg',
      content: (
        <ClientContactCrudPanel
          mode="create"
          clientId={client.id}
          onCompleted={async () => {
            clientQuery.reload();
  ...
  const openEditContactModal = (contact: ApiClientDetail['contacts'][number]) => {
    openModal({
  ...
  const openCreateConsigneeModal = () => {
    openModal({
      title: `Add Consignee to ${client.name}`,
      size: 'lg',
      content: (
        <ConsigneeCrudPanel
          mode="create"
  ...
  const openEditConsigneeModal = (consignee: ApiClientDetail['consignees'][number]) => {
    openModal({
  ...
  const openCreateConsigneeContactModal = (consignee: ApiClientDetail['consignees'][number]) => {
    openModal({
      title: `Add Contact to ${consignee.name}`,
      size: 'lg',
      content: (
        <ConsigneeContactCrudPanel
          mode="create"
  ...
  const openEditConsigneeContactModal = (
    consignee: ApiClientDetail['consignees'][number],
    contact: NonNullable<ApiClientDetail['consignees'][number]['contacts']>[number],
  ) => {
    openModal({
      title: `Edit ${contact.name}`,
      size: 'lg',
        ...
        <div className="space-y-6">
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 p-5">
              ...
              {canManageClients ? (
                <button
                  onClick={openCreateConsigneeModal}
                  className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold uppercase tracking-wider text-slate-700 transition-colors hover:bg-slate-50"
                >
                  <Plus className="h-3.5 w-3.5" />
                  Add Consignee
                  ...
                  <div key={consignee.id} className="rounded-2xl border border-slate-100 p-4">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        {canManageClients ? (
                          <button
                            onClick={() => openCreateConsigneeContactModal(consignee)}
                            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold uppercase tracking-wider text-slate-700 transition-colors hover:bg-slate-50"
                          >
                            <Plus className="h-3.5 w-3.5" />
                            Add Contact
                        ...
                        {canManageClients ? (
                          <button
                            onClick={() => openEditConsigneeModal(consignee)}
                            className="rounded-lg border border-slate-200 bg-white p-2 text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-700"
                            aria-label={`Edit consignee ${consignee.name}`}
                          >
                            <Pencil className="h-3.5 w-3.5" />
                          </button>
                        ) : null}
                    ...
                    {(consignee.contacts || []).length > 0 ? (
                      <div className="mt-4 space-y-2">
                        {(consignee.contacts || []).map((contact) => (
                          <div key={contact.id} className="flex items-start justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-3">
                            <div className="min-w-0">
                            ...
                            {canManageClients ? (
                              <button
                                onClick={() => openEditConsigneeContactModal(consignee, contact)}
                                className="rounded-lg border border-slate-200 bg-white p-2 text-slate-500 transition-colors hover:bg-white hover:text-slate-700"
                                aria-label={`Edit consignee contact ${contact.name}`}
                              >
                                <Pencil className="h-3.5 w-3.5" />
                              </button>
                            ) : null}
                      ...
                      <p className="mt-4 text-xs text-slate-400">No consignee contacts added yet.</p>
                    )}
                  </div>
                ))
              )}
